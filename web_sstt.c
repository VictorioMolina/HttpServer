/*
	Servidor HTTP con conexión TCP persistente
	Realizado por: Victorio Jesús Molina Bermejo
	DNI: 48632380F
	Grupo: 3.2
*/
...
int main(int argc, char **argv)
{
	...
	*/
	...
	debug(LOG, "Arrancando servidor web...", argv[1], getpid());
	...
	while (1)
	{
		...
		if ((socket_fd = accept(listenfd, (struct sockaddr *)&cli_addr, &length)) < 0)
		{
			...
			debug(LOG, "Conexión establecida", "Conexión con el servidor establecida", socket_fd);
		...
		else
		{
			...
			{ // Proceso hijo
				...
				fd_set rfds;
				struct timeval tv = {TIMEOUT, 0}; /* Esperar TIMEOUT segundos*/
				int retval;
				while (1)
				{
					FD_ZERO(&rfds);
					...
					// Si no hay nada en el socket para ser procesado o se ha acabado el TIMEOUT...
...
// Función auxilprintiar para reconocer expresiones regulares
